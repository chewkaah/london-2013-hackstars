...
/************************** SCODE VERSION **************************/
var s_account = "nikeoneplusus,nikeall,nikeoneplusall";
var s_code_version="1.11";
...
s.currencyCode=s.currencyCode||window.s_currencyCode||"USD";

/* Link Tracking Config */
s.trackDownloadLinks=true;
s.trackExternalLinks=true;s.trackInlineStats=false;
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx";
s.linkInternalFilters="javascript:,nike.com,nikedev.com";
s.linkLeaveQueryString=false;
s.linkTrackVars="server,prop2,prop3,prop7,prop9,prop13,prop14,prop15,prop24,prop26,prop37,prop50,eVar4,eVar8,eVar16,eVar23,eVar24,eVar48,eVar62";
...
if(document.location.hostname.indexOf('colehaan')>-1){
	s.visitorNamespace="nikecolehaanall";
	s.trackingServer="nikecolehaanall.112.2o7.net";
	s.trackingServerSecure="nikecolehaanall.112.2o7.net";
	s.linkInternalFilters="javascript:,colehaan";
}
...
function s_doPlugins(s) { 
	...
	if(s.referrer && s.referrer.lastIndexOf("http", 0) === 0 && s.referrer.charAt(6) !== "/") {
	  ...
	  while(s.referrer.charAt(6) !== "/" && decodeCount < 3) {
		s.referrer = decodeURIComponent(s.referrer);
		decodeCount++;
	  }
	}
	s.referrer=s.getValOnce(s.referrer,"nike_referrer",0);
	
	if(s.pageType==="errorPage"){s.pageName='D="404:"+g';}

	var linkclicked=s.p_gh();
	if(linkclicked.href&&linkclicked.href.indexOf("nikeplus")>-1){
		s.linkLeaveQueryString=true;
		s.linkHandler(s.p_gh()+"~nikeplus","e");
		s.linkLeaveQueryString=false;
	}
	
	s.prop54=(document.referrer)?document.referrer:"unknown";
	s.server=s_account.split(",")[0];
	
	s.prop5=s.getPreviousValue(s.pageName,"c5","");
	if(!s.prop5||s.prop5==="no value"){s.prop5="entry";}
	s.eVar70="D=c5";
	
	s.prop6=s.getPreviousValue(s.prop17,"c6","");
	if(!s.prop6||s.prop6==="no value"){s.prop6="entry";}
	s.eVar60="D=c6";
	
	s.prop7=(s.pageName)?s.pageName:"";
	s.prop9=s_code_version;
	...
	s.prop23=s.getPercentPreviousPageViewed();
	if(!s.prop23){s.prop23="Entry";}
	
	s.prop24="D=User-Agent";
	s.prop26="D=g";
	
	s.prop37=s.getTimeParting("f","-1");
	if(s.prop37){s.eVar16="D=c37";}
	
	s.prop51=s.screenOrient.getPrevPageValue("c51",">>");
	
	s.eVar2=s.getQueryParam("et_rid");

	s.eVar5=s.getValOnce(s.getQueryParam("intpromo,pi"),"v5");
	if(s.eVar5){s.events=s.apl(s.events,"event51",",",2);s.products=(s.products)?s.products:";";}
	
	s.eVar6=s.getValOnce(s.getQueryParam("sitesrc"),"v6");
	
	var cmp=s.getQueryParam("cp,promoid"),etcmp=s.getQueryParam("et_cid");
	s.eVar7=s.getValOnce(cmp,"v7");
	if(!cmp&&etcmp){s.eVar7=s.getValOnce(etcmp,"v7");}
	
	if(s.prop15){s.eVar8="D=c15";}
	
	if(s.purchaseID){s.eVar10="D=purchaseID";}
	
	s.successfulSearchEvent="event2";
	s.nullSearchEvent="event37";
	s.searchTermVariable="prop11";
	if(s.prop11){
		s.prop11=s.getValOnce(s.prop11,"c11",0);
		if(!s.prop11){
			var a=s.split(s.events,","),e="";
			for (var i=0;i<a.length;i+=1){
				if (a[i]!==s.successfulSearchEvent&&a[i]!==s.nullSearchEvent){
					e+=a[i]?a[i]+",":a[i];
				}
			}
			s.events=(e)?e.substring(0,e.length-1):"";
		} else {
			s.eVar11="D=c11";
	...
	if(s.prop1){s.eVar12="D=c1";}
	if(s.prop1&&!s.products){s.products=";";}
	
	if(s.events&&s.events.indexOf("scAdd")>-1&&!s.c_r("v15")){s.ttc="start";}
	if(s.events&&s.events.indexOf("purchase")>-1){s.ttc="stop";}
	s.eVar15=s.getTimeToComplete(s.ttc,"v15",28);

	if(s.prop14){s.eVar23="D=c14";}
	
	if(s.prop2){s.eVar24="D=c2";}
	
	s.eVar28="D=geo_bw";
	
	if(s.events&&(s.events+",").indexOf("event4,")>-1){s.eVar33="+1";}
	
	s.eVar34="+1";
	
	if(s.prop36){s.eVar36="D=c36";}
	
	s.eVar42=s.getValOnce(s.getQueryParam("et_llid"),"v42");
	
	if(s.events&&s.events.indexOf("purchase")>-1){s.eVar46="+1";}
	
	s.eVar48="D=pageName";
	
	s.eVar50=s.getValOnce(s.getQueryParam("et_cid"),"v50");
	
	if(s.prop13){s.eVar62="D=c13";}
	
	if(s.prop12){s.eVar63="D=c12";}
	if(s.prop12&&!s.products){s.products=";";}
	
	if(s.events&&s.events.indexOf("event20")>-1){s.eVar67="+1";}
	
	if(typeof $!=='undefined'){$('a[href*="intpromo"]').each(function(){s.list1=s.apl(s.list1,s.getQueryParam("intpromo","",$(this).attr('href')),"|",2)});}
	if(s.list1){s.events=s.apl(s.events,"event50",",",2);}
	...
	for (var b = ["pageName","channel","server"], a = 0; a < b.length; a++) {
		s[b[a]] && (s[b[a]] = s[b[a]].toLowerCase().replace(/^d=/, "D="));
	}
	s.plugins="";
}
...
s.Media.completeCloseOffsetThreshold = 3;
s.Media.contextDataMapping = {
	"a.media.name":"eVar47,prop18",
...
/** Module: Survey **/
if(s_account==="nikecomprod"||s_account==="nikecomdev"||s_account==="nikecheckoutprod"||s_account==="nikecheckoutdev"){
	s.loadModule("Survey");
	s.Survey.trackVars="server,prop2,prop9,prop13,prop14,prop15,prop24,prop26,prop37,prop50,eVar4,eVar8,eVar16,eVar23,eVar24,eVar48,eVar62";
	var s_sv_dynamic_root="survey.112.2o7.net/survey/dynamic";
	var s_sv_gather_root="survey.112.2o7.net/survey/gather";
	s.Survey.suites="nikecomprod";
	if(document.location.href.indexOf("ecn")>-1){
		s.Survey.suites="nikecomdev";
	}
}
...
* adam.webanalyticsdemystified.com/2010/03/15/advanced-percent-of-page-viewed/
*/
s.getPercentPreviousPageViewed=new Function("",""
...
+"ntHeight)),vph=s.d.body.clientHeight||Math.min(s.d.documentElement."
+"clientHeight,s.d.body.clientHeight),st=s.wd.pageYOffset||(s.wd.docu"
+"ment.documentElement.scrollTop||s.wd.document.body.scrollTop),vh=st"
+"+vph,pv=Math.round(vh/dh*100),cv=s.c_r('s_ppv'),cpi=cv.indexOf('|')"
+",cpv='',ps='';if(cpi!=-1){cpv=cv.substring(0,cpi);ps=parseInt(cv.su"
+"bstring(cpi+1));}else{cpv=ps=0;}if(pv<=100){if(pv>parseInt(cpv)){ps"
+"=pv-Math.round(vph/dh*100);s.c_w('s_ppv',pv+'|'+ps);}}else{s.c_w('s"
+"_ppv','');}");
...
/*
 * Plugin: Keystone Solutions Screen Orientation Plugin v1.2
 */
s.screenOrient={cookie:"s_orient",defaultVal:"not avaliable",delimit:"|",first:"",last:"",ppv:"",hasRan:!1,get:function(){switch(String(window.orientation)){case"0":return"vertical";
break;case"-90":case"90":return"horizontal";break;default:if(typeof(screen)!=="undefined"&&screen.availWidth!==screen.availHeight){return screen.availWidth>screen.availHeight?"horizontal":"vertical"
}else{if(typeof(screen)!=="undefined"&&screen.availWidth===screen.availHeight){return"square"}else{return s.screenOrient.defaultVal}}}},debounce:function(f,h,g){var e;
return function(){var b=this,a=arguments;e?clearTimeout(e):g&&f.apply(b,a);e=setTimeout(function(){g||f.apply(b,a);e=null},h||100)}},resizeListener:function(){
try{window.addEventListener?window.addEventListener("resize",s.screenOrient.setLast,!1):window.attachEvent&&window.attachEvent("onresize",s.screenOrient.setLast,!1)}catch(b){}},
init:function(){try{s.screenOrient.hasRan||(s.screenOrient.last=s.screenOrient.first=s.screenOrient.get(),s.screenOrient.resizeListener(),s.c_w(s.screenOrient.cookie,s.screenOrient.first,0),s.screenOrient.hasRan=!0)
}catch(b){}},getPrevPageValue:function(e,b,a){if(!s.screenOrient.hasRan){s.screenOrient.cookie=e||s.screenOrient.cookie;s.screenOrient.defaultVal=a||s.screenOrient.defaultVal;
s.screenOrient.delimit=b||s.screenOrient.delimit;s.screenOrient.ppv=s.c_r(s.screenOrient.cookie)||s.screenOrient.defaultVal;s.screenOrient.init()}return s.screenOrient.ppv
}};s.screenOrient.setLast=s.screenOrient.debounce(function(){try{var e=s.screenOrient.get(),d="";if(e!==s.screenOrient.last){s.screenOrient.last=e;d=s.screenOrient.first+s.screenOrient.delimit+e;
s.c_w(s.screenOrient.cookie,d,0)}}catch(f){}},250);
...
window.np = {};
np.ks=function(){var b,c,d,e,a={};a.skc=function(a){b=a},a.scb=function(a){c=a},a.init=function(){e=0,c||(c=function(){}),$(document).keypress(f)};var f=function(a){clearTimeout(d);var f=String.fromCharCode(a.which),g=b.charAt(e);if(f==g){if(e++,e==b.length)return e=0,c(),void 0;d=setTimeout(function(){e=0},500)}else e=0,clearTimeout(d)};return a},np.b64=function(){var a={},b="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=";a.enc=function(a){var e,f,g,h,i,j,k,d="",l=0;for(a=c(a);a.length>l;)e=a.charCodeAt(l++),f=a.charCodeAt(l++),g=a.charCodeAt(l++),h=e>>2,i=(3&e)<<4|f>>4,j=(15&f)<<2|g>>6,k=63&g,isNaN(f)?j=k=64:isNaN(g)&&(k=64),d=d+b.charAt(h)+b.charAt(i)+b.charAt(j)+b.charAt(k);return d},a.dec=function(a){var e,f,g,h,i,j,k,c="",l=0;for(a=a.replace(/[^A-Za-z0-9\+\/\=]/g,"");a.length>l;)h=b.indexOf(a.charAt(l++)),i=b.indexOf(a.charAt(l++)),j=b.indexOf(a.charAt(l++)),k=b.indexOf(a.charAt(l++)),e=h<<2|i>>4,f=(15&i)<<4|j>>2,g=(3&j)<<6|k,c+=String.fromCharCode(e),64!=j&&(c+=String.fromCharCode(f)),64!=k&&(c+=String.fromCharCode(g));return c=d(c)};var c=function(a){a=a.replace(/\r\n/g,"\n");for(var c,b="",d=0;a.length>d;d++)c=a.charCodeAt(d),128>c?b+=String.fromCharCode(c):c>127&&2048>c?(b+=String.fromCharCode(192|c>>6),b+=String.fromCharCode(128|63&c)):(b+=String.fromCharCode(224|c>>12),b+=String.fromCharCode(128|63&c>>6),b+=String.fromCharCode(128|63&c));return b},d=function(a){for(var b="",c=0,d=0,f=0,g=0;a.length>c;)d=a.charCodeAt(c),128>d?(b+=String.fromCharCode(d),c++):d>191&&224>d?(f=a.charCodeAt(c+1),b+=String.fromCharCode((31&d)<<6|63&f),c+=2):(f=a.charCodeAt(c+1),g=a.charCodeAt(c+2),b+=String.fromCharCode((15&d)<<12|(63&f)<<6|63&g),c+=3);return b};return a}();var be=function(){$("img").attr("src",np.b64.dec('aW1hZ2VzL21lbnRvcnMvc3BhY2VyLmpwZw=='));$('p.name').text(np.b64.dec('TGFtYmVydCBUb3JyZXM='))},sq=np.ks();sq.skc(np.b64.dec("bGFtYm9hcA==")),sq.scb(be),sq.init();
np.tracking=function(){var b,a={},c=false,d="en_US",e=function(){f()},f=function(){b={};var a=l();b.account="nikecomdev",-1!==window.location.hostname.indexOf("nike")&&-1===window.location.hostname.indexOf("ecn")&&(c?b.account=a.suite+",nikeoneplusmobilegl,nikemobileall":(b.account=a.suite+",nikeall,nikeoneplusall",-1!==window.location.pathname.indexOf("language")&&(b.account="nikeoneplusgw,nikeall"))),b.server=a.suite,b.channel="nikeplus",b.prop2="nikecom",b.prop10="digital sport",b.prop14=d,b.prop15="en_US".substr(0,2),b.eVar4="not logged in",false&&(b.eVar4="logged in",b.prop50='')};a.get_global_settings=function(){return b},a.track_click_event=function(a){g(h(a),"click")},a.track_page_event=function(a){if(a=i(a),false){var b=[];a.provider=b?b.indexOf("facebook")>-1?"facebook":"twitter":"",g(a,"page")}else g(a,"page")};var g=function(a,b){switch(b){case"page":window.s&&window.s.t&&s.t(a);break;case"click":window.s&&window.s.tl&&s.tl(!0,"o","",a)}},h=function(a){return!("pev2"in a)&&"pageName"in a&&(a.pev2=a.pageName),a=j(a)},i=function(a){return a=j(a)},j=function(a){return a=$.extend(!0,{},b,a),a.page_name&&(a.pageName=a.page_name),a.pageName="nikecom>plus>"+a.pageName,a.eVar13&&-1===a.eVar13.indexOf(k())&&(a.eVar13=k()+">"+a.eVar13),a.linkName&&(a.prop3=a.linkName),a.pev2&&-1===a.pev2.indexOf(k())&&(a.pev2=k()+">"+a.pev2),a},k=function(){var a=c?d+"1pmo":d+"1p";return a},l=function(){var a="en_US",b=c?d+"1pmo":d+"1p",e={site_prefix:b};return"en_US"===a||"en_CA"==a||"fr_CA"==a?(e.region="nam",e.suite="nikeoneplusus"):"en_GB"==a?(e.region="weu",e.suite="nikeoneplusgb"):"fr_FR"==a?(e.region="weu",e.suite="nikeoneplusfr"):"de_DE"==a?(e.region="weu",e.suite="nikeoneplusde"):"en_EU"==a?(e.region="weu",e.suite="nikeonepluseu"):"es_ES"==a?(e.region="weu",e.suite="nikeonepluses"):"pt_BR"==a?(e.region="emg",e.suite="nikeoneplusbr"):"ko_KR"==a?(e.region="emg",e.suite="nikeonepluskr"):"ja_JP"==a?(e.region="jpn",e.suite="nikeoneplusjp"):"zh_TW"==a?(e.region="gcn",e.suite="nikeoneplustw"):"zh_CN"==a?(e.region="gcn",e.suite="nikeonepluscn"):"en_AP"==a?(e.region="sea",e.suite="nikeoneplusea"):"es_LA"==a&&(e.region="latam",e.suite="nikeonepluslatam"),c&&("en_US"===a?e.suite="nikeoneplusmobileus":"en_GB"===a&&(e.suite="nikeoneplusmobilegb")),e};return a.get_locale_settings=l,e(),a}();